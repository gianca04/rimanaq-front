...
class GestureCaptureSystem {
  constructor() {
    ...
    // Para edición, no cargamos gestos desde localStorage
    this.savedGestures = [];
  ...
  initializeEventListeners() {
    ...
    // Eventos específicos para edición
    document
      .getElementById("exportCurrentBtn")
      ?.addEventListener("click", () => this.exportCurrentGesture());
  ...
  // Función simplificada para limpiar frames actuales
  clearCurrentFrames() {
    this.uiManager.clearCurrentFrames();
  }

  // Función para exportar el gesto actual que se está editando
  exportCurrentGesture() {
    this.dataManager.exportCurrentGesture();
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Disparar evento cuando el sistema esté listo
  setTimeout(() => {
    window.dispatchEvent(new CustomEvent('gestureSystemReady', {
      detail: { gestureSystem: gestureSystem }
    }));
  }, 500);
});
